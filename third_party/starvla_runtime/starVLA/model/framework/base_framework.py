...
class baseframework(PreTrainedModel):
    ...
    def supports_training_tag(self, tag: str) -> bool:
        ...
        return tag == "vla" and type(self).forward is not baseframework.forward
    ...
    @classmethod
    def _apply_config_overrides(cls, model_config: dict, config_overrides=None) -> dict:
    ...
    @staticmethod
    def _load_state_dict_compatible(model, model_state_dict: dict):
        ...
        for key, value in model_state_dict.items():
            if key in current_state and tuple(current_state[key].shape) == tuple(value.shape):
                ...
                expected_shape = tuple(current_state[key].shape) if key in current_state else None
                skipped.append((key, tuple(value.shape), expected_shape))

        missing_keys, unexpected_keys = model.load_state_dict(compatible_state, strict=False)
        logger.warning(
            "Partial checkpoint load enabled: loaded %d tensors, skipped=%d, missing_after_partial=%d, unexpected=%d",
        ...
        if skipped:
            preview = ", ".join(f"{key}:{shape}->{expected}" for key, shape, expected in skipped[:8])
            logger.warning("Partial load skipped preview: %s", preview)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model_config = cls._apply_config_overrides(model_config, config_overrides=config_overrides)
        ...
        try:
            if allow_partial_load:
                FrameworkModel = cls._load_state_dict_compatible(FrameworkModel, model_state_dict)
            else:
